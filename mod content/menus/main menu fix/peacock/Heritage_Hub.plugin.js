				...
				actions: {
					activated: [
						...
						},
						{
							$if: {
								$condition: {
									$and: ["$isuser freeprologue", "$isininventory ACCESS_HIT_FLAMINGO"]
								},
								$then: {
									"set-selected": {
										target: "FSP_Buy"
						...
						$if: {
							$condition: {
								$and: [
									"$not $isonline",
									{
										$or: ["$not $eq($platform,izumo)", "$not $isuser freeprologue"]
									}
								]
							},
							$then: {
					...
					actiony: {
						$if: {
							$condition: {
								$and: ["$eq ($platform,pc)", "$isuser freeprologue"]
							},
							$then: {
								$include: {
										...
										"$if $isuser freeprologue": {
											$then: {
												"$if $eq($platform,izumo)": {
													$then: {
														$include:
															"menusystem/pages/hub/dashboard/dashboardfreestarterpack_izumo.json"
													},
													$else: {
														"$if $not $isonline": {
															$then: {
																$include:
																	"menusystem/pages/hub/dashboard/dashboardfreestarterpack_offline.json"
															},
															$else: {
																$include:
																	"menusystem/pages/hub/dashboard/dashboardfreestarterpack.json"
															}
														}
													}
												}
