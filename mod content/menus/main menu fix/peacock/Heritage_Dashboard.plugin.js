...
module.exports = function HUI_PEACOCK_MENU_DASHBOARD(controller) {
	...
	function crc32(t) {
		...
		for (let t = 0; t < 256; t++) {
			let r = t
			for (let t = 0; t < 8; t++) 1 & r ? (r = (r >>> 1) ^ 3988292384) : (r >>>= 1)
			e[t] = r
		}
					...
					$if: {
						$condition: {
							$and: [
								"$isuser vr",
								{
									$or: ["$isvrdeviceavailable", "$eq ($platform,orbis)"]
								}
							]
						},
						$then: [
	...
	if (!controller.smf.modIsInstalled("BurnthewitchRiisikumi.Roulette")) {
		{
			...
			controller.configManager.configs.EiderDashboard.children.$mergearrays[3].data.image =
				"$res images/challenges/profile_challenges/escalation_s2_tier_9.jpg"
			var Roulettetile = [
				{
